...
//             <span className="text-[10px] font-medium grow-0  ">
//               {isRTL ? "أسم الموقع" : "Site Name"} :
//             </span>
//             <span className="text-[10px] md:text-xs font-bold text-gray-900 grow line-clamp-1">
//               {item.name ?? ""}
//             </span>
//           </div>
...
export default function LocationCard({ item, isNotHome }: LocationCardProps) {
  ...
  const [deleteLocation, { isLoading }] =
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-50
        ...
        hover:shadow-[var(--shadow-primary-sm)] transition-colors"
      >
        {/* HEADER */}
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg
            bg-(--primary) text-white text-[10px] font-bold shrink-0">
            <Building2 size={12} />
          ...
          {item.status && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full
              bg-green-50 text-green-600 text-[10px] font-bold
              border border-green-100 shrink-0">
              {item.status}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-start gap-2">
            ...
            <span className="text-[10px] md:text-xs font-medium lg:font-bold text-gray-900
              grow line-clamp-1">
              {item.name ?? ""}
            </span>
          </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <span className="text-[10px] md:text-xs font-medium lg:font-bold text-gray-900 grow line-clamp-1">
              {item.workers_count ?? 0} {isRTL ? "عامل" : "Staff"}
            </span>
          </div>
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-start gap-2">
              <FaCity size={14} className="shrink-0" />
              <span className="text-[10px] font-medium shrink-0">
                {isRTL ? "المدينة" : "City"} :
              </span>
              <span className="text-[10px] md:text-xs font-medium lg:font-bold text-gray-900
            ...
            {isNotHome && (
              <Button
                variant="primary"
                className="shrink-0 bg-(--warning-text)/20
                  text-(--warning-text) hover:bg-(--warning-text)/30
                  border border-(--warning-text) text-xs h-7"
