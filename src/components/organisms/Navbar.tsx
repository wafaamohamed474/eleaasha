...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { name: t("contact"), href: "https://wa.me/+966500000000", id: "contact", isExternal: true },
  ];
 ...
 {/* Right Side: Logo */}
        <div className="relative z-50">
          <Link href="/">
            <Image src={logo} alt="El-Eaasha Logo"   className="w-20 h-20 object-contain" />
          </Link>
        </div>
        ...
        <div className="hidden lg:flex items-center space-x-8  ">
          {navLinks.map((link) => (
            <Link
              ...
              onClick={() => { if(!link.isExternal) setActiveSection(link.id) }}
            >
              {link.name}
            </Link>
          ))}
